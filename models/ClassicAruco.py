...
class ClassicAruco:
    def __init__(self, max_error=4, use_original=True, use_flat_field=True, use_gamma=True, gamma_val=3.0):
    ...
    def detect(self, image_input):
        """Processes a single image (path or cv2 object) and returns a list of (id, x, y) tuples."""
        ...
        if self.use_original:
            image_variations.append(gray_orig)
        if self.use_flat_field:
            image_variations.append(apply_flat_field(gray_orig))
        if self.use_gamma:
            image_variations.append(apply_gamma_correction(gray_orig, gamma=self.gamma_val))
        
        found_markers = {}
        
        for gray in image_variations:
            for win_size in range(3, 39, 6):
                thresh = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, win_size, 5)
                contours, _ = cv2.findContours(thresh, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
                
                for cnt in contours:
                    area = cv2.contourArea(cnt)
                    if area < 75 or area > (gray.shape[0] * gray.shape[1] * 0.8):  
                        continue
                    ...
                    if len(approx) == 4 and cv2.isContourConvex(approx):
                        ...
                        warped = cv2.warpPerspective(gray, M, (160, 160))
                        ...
                        if min_error > self.max_error:
                            ...
                            if is_valid_border(warped_adapt):
                                ...
                                if adapt_err < min_error:
                                    min_error = adapt_err
                                    best_match_id = adapt_id
                                    best_rotation = adapt_rot
    ...
    def detect_dataset(self, image_dir, output_csv):
        ...
        with open(output_csv, mode='w', newline='', encoding='utf-8') as f:
            ...
            for filename in sorted(os.listdir(image_dir)):
                ...
                raw_results = self.detect(image_path)
